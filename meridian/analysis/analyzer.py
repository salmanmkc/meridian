...
class DataTensors(backend.ExtensionType):
  ...
  def validate_and_fill_missing_data(
    ...
    Args:
      ...
      model_context: The ModelContext object to validate against and get the
        original data tensors from.
    ...
    self._validate_correct_variables_filled(
        required_tensors_names, model_context
    )
    self._validate_geo_dims(required_tensors_names, model_context)
    self._validate_channel_dims(required_tensors_names, model_context)
    if allow_modified_times:
      self._validate_time_dims_flexible_times(
          required_tensors_names, model_context
      )
    else:
      self._validate_time_dims(required_tensors_names, model_context)

    return self._fill_default_values(required_tensors_names, model_context)
  ...
  def _validate_correct_variables_filled(
    ...
    Args:
      required_variables: A sequence of data tensors names that are required to
        be filled in.
      model_context: The ModelContext object to validate against.
  ...
  def _validate_geo_dims(
      self, required_fields: Sequence[str], model_context: context.ModelContext
  ):
  ...
  def _validate_time_dims_flexible_times(
    ...
    new_n_times = self.get_modified_times(model_context)
...
class Analyzer:
  ...
  def incremental_outcome(
    ...
    new_n_media_times = data_tensors.get_modified_times(self._model_context)
  ...
  def summary_metrics(
    ...
    if not aggregate_times:
      ...
      modified_times = new_data.get_modified_times(self._model_context)
    ...
    if new_data.get_modified_times(self._model_context) is None:
    ...
    ).validate_and_fill_missing_data(constants.SPEND_DATA, self._model_context)
  ...
  def get_aggregated_impressions(
    ...
    if new_data is None:
      new_data = DataTensors()
    data_tensors = new_data.validate_and_fill_missing_data(
        tensor_names_list, self._model_context
    )
    n_times = (
        data_tensors.get_modified_times(self._model_context)
  ...
  def optimal_freq(
    ...
    n_media_times = (
        filled_data.get_modified_times(self._model_context)
  ...
  def response_curves(
    ...
    new_n_media_times = filled_data.get_modified_times(self._model_context)
  ...
  def get_aggregated_spend(
    ...
    filled_data = new_data.validate_and_fill_missing_data(
        required_tensors_names, self._model_context
    )
